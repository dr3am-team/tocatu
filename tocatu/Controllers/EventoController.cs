...
namespace tocatu.Controllers
{
    public class EventoController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("EventId,Nombre,Descripcion,PrecioEntrada,Dia,Hora, BarId")] Evento evento, [Bind("PhotoAvatar")]IFormFile imagen)
        {
        ...
        public IActionResult GetImage(int id)
        {
            ...
            if (evento1 != null)
            {
                ...
                string fullPath = webRootpath + folderPath + evento1.ImageName;
                if (System.IO.File.Exists(fullPath))
                {
                    ...
                    return File(fileBytes, evento1.ImageMimeType);
                }
                else
                {
                    if (evento1.PhotoFile.Length > 0)
                    {
                        return File(evento1.PhotoFile, evento1.ImageMimeType);
                    }
                    else
                    {
                        return NotFound();
